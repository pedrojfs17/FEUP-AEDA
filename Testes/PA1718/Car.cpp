...
bool Car::operator==(const Car &car) const {
    std::cout << this->brand << " == " << car.brand << " && " << this->model << " == " << car.model << std::endl;
...
float ElectricCar::range() const {
    return battery * (100.0/kWh_100km);
}
...
float HybridCar::range() const {
    return this->getTank() * (100.0/this->getL_100km()) + this->getBattery() * (100.0/this->getKWh_100km());
}
